...
class motional_distribution(object):
    ...
    def _displaced_thermal(cls, alpha, nbar, n):
        ...
        try:
            ...
            populations = 1. / (nbar + 1.0) * (nbar / (nbar + 1.0)) ** n * laguerre(n, 0, -alpha ** 2 / (
                        nbar * (nbar + 1.0))) * np.exp(-alpha ** 2 / (nbar + 1.0))
        except FloatingPointError:
            np.seterr(**old_settings)
            print('precise calculation required', alpha, nbar)
            populations = [mp.fprod((1. / (nbar + 1.0), mp.power(nbar / (nbar + 1.0), k),
